...
# --- Constants for Code 128-B Encoding ---
_START_CODE_B_VALUE = 104
_START_CODE_A_VALUE = 103 # Added for reverse mapping
_START_CODE_C_VALUE = 105 # Added for reverse mapping

_STOP_CODE_VALUE = 106
_MAX_INPUT_LENGTH = 80 # Arbitrary limit to prevent excessively long barcodes

# Mapping of ASCII characters (32-127) to Code 128 values (0-95).
_CODE_128_B_CHAR_TO_VALUE = {chr(i): i - 32 for i in range(32, 128)}

# Code 128 Patterns (bar_width, space_width, bar_width, space_width, bar_width, space_width).
# For values 0-105, the sum of widths of bars is 7, and the sum of widths of spaces is 4, totaling 11 modules.
# For the Stop character (value 106), the pattern is (2,3,3,1,1,1) followed by a 2-module termination bar.
_CODE_128_PATTERNS = {
    ...
    30: (2,1,2,1,2,3), 31: (2,1,2,3,2,1), 32: (2,3,2,1,2,1), 33: (1,1,1,2,2,3), 34: (1,1,1,3,2,2),
    35: (1,1,2,2,1,3), 36: (1,1,2,3,1,2), 37: (1,2,1,1,2,3), 38: (1,2,1,3,1,2), 39: (1,3,1,1,2,2),
    40: (1,3,1,2,1,2), 41: (1,1,2,1,3,2), 42: (1,1,2,2,3,1), 43: (1,2,1,1,3,2), 44: (1,2,1,2,3,1),
    45: (1,2,3,1,1,2), 46: (1,2,3,2,1,1), 47: (1,1,3,2,1,2), 48: (1,2,3,1,2,1), 49: (1,1,2,1,2,3),
    50: (1,1,2,3,2,1), 51: (1,3,2,1,2,1), 52: (1,1,1,1,4,2), 53: (1,1,1,2,4,1), 54: (1,1,1,4,2,1),
    55: (1,4,1,1,1,2), 56: (1,4,1,2,1,1), 57: (1,1,2,1,4,1), 58: (1,1,4,1,2,1), 59: (1,2,1,1,4,1),
    ...
    106: (2,3,3,1,1,1), # Stop (followed by 2-module termination bar)

}
_PATTERN_TO_CODE_128_VALUE = {}

# Initialize reverse mapping, prioritizing data characters and start/stop codes
for value, pattern in _CODE_128_PATTERNS.items():
    if 0 <= value <= 95 or value in [_START_CODE_B_VALUE, _STOP_CODE_VALUE, _START_CODE_A_VALUE, _START_CODE_C_VALUE]: # Prioritize data chars and start/stop
...
class BarcodeTest:
    ...
    def run_all_tests(self):
        ...
        self.test_encode_string_boundary_chars() # New test
    ...
    def test_encode_string_default(self):
        ...
        self.assert_true(len(values) == len(expected_values) + 3, f"Length of encoded values (default): {len(values)} vs {len(expected_values) + 3}") # Start, Data, Checksum, Stop        
    ...
    def test_checksum_calculation(self):
        ...
        input_str = "Agentic AI"
        encoded_values = [_CODE_128_B_CHAR_TO_VALUE[c] for c in input_str] # Values for 'Agentic AI'
        ...
        while idx < len(parsed_module_widths):
            ...
            if idx + 6 <= len(parsed_module_widths):
                ...
                if found_value is not None:
                    ...
                    self.failed_tests += 1
                    print(f"FAIL: Simulated decodability - Unrecognized 6-element pattern at index {idx}: {current_pattern_tuple}. Original values: {original_encoded_values}, Decoded so far: {decoded_values}")
